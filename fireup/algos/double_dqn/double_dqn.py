...
import gym
import time
from fireup.algos.double_dqn import core
...
Double Deep Q-Network from https://arxiv.org/abs/1509.06461

"""
def double_dqn(env_fn,
    ...
    for t in range(total_steps):
        ...
        if replay_buffer.size > min_replay_history and t % update_period == 0:
            ...
            # q_pi_targ, _ = target(obs2).max(1)  # Vanilla
            next_act_idx = main(obs2).argmax(-1, keepdim=True)  # double dqn
            q_pi_targ = target(obs2).gather(1, next_act_idx).squeeze()  # double dqn
