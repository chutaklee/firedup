...
import gym
import time
from fireup.algos.dqn_atari import core, atari_wrappers
from fireup.utils.logx import EpochLogger


class ImageToNp(gym.ObservationWrapper):
    """Image shape to num_channels x weight x height"""
    def __init__(self, env):
        super(ImageToNp, self).__init__(env)
...
def wrap_env(env):
    assert 'NoFrameskip' in env.spec.id, 'NoFrameskip env is required'
...
def dqn_atari(
    ...
    env, test_env = wrap_env(env), wrap_env(test_env)
    ...
    def get_action(o, epsilon):
        ...
        if np.random.random() <= epsilon:
            ...
            q_values = main(torch.Tensor(o).unsqueeze(0).to(device))
